...
class Forecast_Task_Manager
{
	public:
		...
		/* zxliu added */
		size_t		count_idle_threads() 					{ return job_service_->count_idle_threads(); }
		size_t		tp_size() 								{ return job_service_->tp_size(); }
		int 		curtail_threadpool(int curtail_size)	{ return job_service_->curtail_threadpool(curtail_size); }
		int 		extend_threadpool(int extend_size) 		{ return job_service_->extend_threadpool(extend_size);	}
		void		mutex_size_lock() 						{ job_service_->mutex_size_lock(); } 
		void 		mutex_size_unlock()						{ job_service_->mutex_size_unlock(); }
		void		get_tp_thrput_diff() 					{ job_service_->get_tp_thrput_diff(); }
		Ads_Job_Queue_Service_TP_Adaptive::TP_Throughput* tp_thrput() { return job_service_->tp_thrput(); }
	...
	private:
		...
		/* zxliu revised */
		Ads_Job_Queue_Service_TP_Adaptive* job_service_; // build-in job_queue_service, used to handle all the tasks
